...
class VideoConfig:
    ...
    def kr_font_size(self) -> int:
        return 58 if self.is_shorts else 68

    @property
    def target_font_size(self) -> int:
        return 82 if self.is_shorts else 92

    @property
    def tip_font_size(self) -> int:
        return 36 if self.is_shorts else 42
...
def layout_text_block(kr_clip, target_clip, tip_clip, video_size: Tuple[int, int]):
    ...
    gap1 = int(h * 0.025)
    gap2 = int(h * 0.018)
    total = kr_clip.h + gap1 + target_clip.h + gap2 + tip_clip.h
    top = int(h * 0.48 - total / 2)
    top = max(top, int(h * 0.12))

    return (
        ("center", top),
        ("center", top + kr_clip.h + gap1),
        ("center", top + kr_clip.h + gap1 + target_clip.h + gap2),
    )
...
def make_progress_clip(idx: int, total: int, cfg: VideoConfig, duration: float):
    ...
    x = cfg.size[0] - clip.w - int(cfg.size[0] * 0.05)
    y = int(cfg.size[1] * 0.045)
...
def make_illustration_background(topic: str, cfg: VideoConfig, idx: int, total: int) -> Image.Image:
    w, h = cfg.size
    p = PALETTES.get(cfg.illustration_style, PALETTES["warm"])
    img = _gradient((w, h), p["top"], p["bottom"])

    # soft atmosphere
    _draw_soft_blob(img, (int(w*-0.15), int(h*0.03), int(w*0.48), int(h*0.34)), p["accent"]+(110,), blur=65)
    _draw_soft_blob(img, (int(w*0.62), int(h*0.56), int(w*1.20), int(h*0.96)), p["accent2"]+(95,), blur=75)
    _draw_soft_blob(img, (int(w*0.10), int(h*0.78), int(w*0.54), int(h*1.12)), (255,255,255,45), blur=60)

    draw = ImageDraw.Draw(img, "RGBA")

    # subtle pattern
    for i in range(26):
        x = int((i * 167 + idx * 53) % w)
        y = int((i * 251 + idx * 79) % h)
        r = 3 + (i % 4) * 2
        draw.ellipse((x-r, y-r, x+r, y+r), fill=(255,255,255,45))

    # central glass card behind subtitles
    _rounded(
        draw,
        (int(w*0.065), int(h*0.37), int(w*0.935), int(h*0.62)),
        54,
        p["glass"],
        (255,255,255,70),
        2,
    ...
    scene = _scene_kind(topic)
    if scene == "airport":
        _draw_airport(draw, w, h, p)
    elif scene == "cafe":
        _draw_cafe(draw, w, h, p)
    elif scene == "travel":
        _draw_travel(draw, w, h, p)
    elif scene == "dm":
        _draw_dm(draw, w, h, p)
    elif scene == "school":
        _draw_school(draw, w, h, p)
    elif scene == "food":
        _draw_food(draw, w, h, p)
    else:
        _draw_language(draw, w, h, p)

    # vignette for text readability
    vignette = Image.new("RGBA", (w, h), (0,0,0,0))
    vd = ImageDraw.Draw(vignette)
    vd.rectangle((0, 0, w, int(h*0.10)), fill=(0,0,0,65))
    vd.rectangle((0, int(h*0.88), w, h), fill=(0,0,0,75))
...
def make_title_box_clip(topic: str, cfg: VideoConfig, duration: float, intro: bool = False):
    w, h = cfg.size
    box_w = int(w * (0.68 if cfg.is_shorts else 0.48))
    box_h = int(h * (0.115 if intro else 0.085))
    img = Image.new("RGBA", (box_w, box_h), (0, 0, 0, 0))
    draw = ImageDraw.Draw(img, "RGBA")
    draw.rounded_rectangle((0, 0, box_w-1, box_h-1), radius=int(box_h*0.34), fill=(255,255,255,218), outline=(255,255,255,248), width=2)
    # accent line
    draw.rounded_rectangle((int(box_w*0.045), int(box_h*0.14), int(box_w*0.055), int(box_h*0.86)), radius=6, fill=(122,162,255,255))
    label_font = get_safe_font("ko", int(box_h * (0.18 if intro else 0.21)))
    title_font = get_safe_font("ko", int(box_h * (0.25 if intro else 0.28)))
    topic_text = topic if len(topic) <= 18 else topic[:18] + "…"
    draw.text((int(box_w*0.10), int(box_h*0.16)), cfg.title_label, font=label_font, fill=(90,105,135,255))
    draw.text((int(box_w*0.10), int(box_h*0.44)), topic_text, font=title_font, fill=(40,55,88,255))
    clip = _with_duration(ImageClip(np.array(img)), duration)
    x = int((w - box_w) / 2)
    y = int(h * (0.055 if not intro else 0.40))
    return _with_position(clip, (x, y))


def make_intro_caption_clip(cfg: VideoConfig, duration: float):
    text = "Speak · Repeat · Learn"
    clip = make_text_image_clip(
        text=text,
        lang="en",
